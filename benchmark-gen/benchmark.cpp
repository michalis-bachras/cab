...
int main(int argc, char **argv)
{
   ...
   std::filesystem::path query_streams_dir = current_dir / "query_streams";
   std::filesystem::create_directories(query_streams_dir);
   for (uint32_t i = 0; i<generator.databases.size(); i++) {
      std::filesystem::path file_path = query_streams_dir / ("query_stream_" + std::to_string(i) + ".json");
